...
class CartesianValidOpTest < Minitest::Test
  def setup
    @factory = RGeo::Cartesian.simple_factory(uses_lenient_assertions: true)
  ...
  def test_invalid_line_string_too_few_points
    ls = @factory.line_string([@point1])
    assert_equal(RGeo::ImplHelper::TopologyErrors::TOO_FEW_POINTS, ls.invalid_reason)
    assert_equal(false, ls.valid?)
  end
  ...
  def test_invalid_linear_ring_too_few_points
    lr = @factory.linear_ring([@point1, @point2, @point1])
    assert_equal(RGeo::ImplHelper::TopologyErrors::TOO_FEW_POINTS, lr.invalid_reason)
    assert_equal(false, lr.valid?)
  end

  def test_invalid_linear_ring_self_intersection
    lr = @factory.linear_ring([@point1, @point2, @point3, @point4])
    assert_equal(RGeo::ImplHelper::TopologyErrors::RING_SELF_INTERSECTION, lr.invalid_reason)
    assert_equal(false, lr.valid?)
  end
  ...
  def test_invalid_polygon_too_few_points
    lr = @factory.linear_ring([@point1, @point2, @point1])
    poly = @factory.polygon(lr)

    assert_equal(RGeo::ImplHelper::TopologyErrors::TOO_FEW_POINTS, poly.invalid_reason)
    assert_equal(false, poly.valid?)

    poly2 = @factory.polygon(@big_square, [lr])
    assert_equal(RGeo::ImplHelper::TopologyErrors::TOO_FEW_POINTS, poly2.invalid_reason)
    assert_equal(false, poly2.valid?)
  end
  ...
  def test_invalid_polygon_self_intersecting_ring
    ...
    assert_equal(RGeo::ImplHelper::TopologyErrors::RING_SELF_INTERSECTION, poly.invalid_reason)
    assert_equal(false, poly.valid?)
  end
  ...
  def test_multipolygon_consistent_area
