      ...
      class WKTParser # :nodoc:
        ...
        def parse(containing_type_ = nil) # :nodoc:
          ...
          unless @cur_token.is_a?(TypeString)
            fail Error::ParseError("Found token #{@cur_token} when we expected a value")
          end
          type_ = @cur_token
          next_token
          ...
          when "TOWGS84"
            ...
            unless bursa_wolf_params_.size == 7
              fail Error::ParseError("Expected 7 Bursa Wolf parameters but found #{bursa_wolf_params_.size}")
            end
          ...
          when "LOCAL_CS"
            ...
            unless axes_.size > 0
              fail Error::ParseError("Expected at least one AXIS in a LOCAL_CS")
            end
          ...
          when "GEOCCS"
            ...
            unless axes_.size == 0 || axes_.size == 3
              fail Error::ParseError("GEOCCS must contain either 0 or 3 AXIS parameters")
            end
          ...
          when "GEOGCS"
            ...
            unless axes_.size == 0 || axes_.size == 2
              fail Error::ParseError("GEOGCS must contain either 0 or 2 AXIS parameters")
            end
          ...
          when "PROJCS"
            ...
            axes_ = args_.find_all(AxisInfo)
            unless axes_.size == 0 || axes_.size == 2
              fail Error::ParseError("PROJCS must contain either 0 or 2 AXIS parameters")
            end
            ...
            fail Error::ParseError, "Unrecognized type: #{type_}"
          end
          args_.assert_empty
          obj_
        end
        ...
        def expect_token_type(type_) # :nodoc:
          unless type_ === @cur_token
            fail Error::ParseError, "#{type_.inspect} expected but #{@cur_token.inspect} found."
          end
        end
        ...
        def next_token # :nodoc:
          ...
          when "", nil
            ...
            if token_ =~ /^[-+]?(\d+(\.\d*)?|\.\d+)(e[-+]?\d+)?$/
              ...
              fail Error::ParseError, "Bad token: #{token_.inspect}"
            end
          end
          @cur_token
        end
        ...
        class ArgumentList # :nodoc:
          ...
          def assert_empty # :nodoc:
            if @values.size > 0
              ...
              fail Error::ParseError, "#{@values.size} unexpected arguments: #{names_.join(', ')}"
            end
          end
          ...
          def shift(klass_ = nil) # :nodoc:
            val_ = @values.shift
            unless val_
              fail Error::ParseError, "No arguments left... expected #{klass_}"
            end
            if klass_ && !val_.is_a?(klass_)
              fail Error::ParseError, "Expected #{klass_} but got #{val_.class}"
            end
            val_
          end
        end
