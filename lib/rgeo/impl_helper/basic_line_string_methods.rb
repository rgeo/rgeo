  ...
  module ImplHelper # :nodoc:
    module BasicLineStringMethods # :nodoc:
      def initialize(factory_, points_)
        ...
        @points = points_.map do |elem_|
          ...
          fail Error::InvalidGeometry, "Could not cast #{elem_}" unless elem_
          elem_
        end
        _validate_geometry
      end
      ...
      def _validate_geometry
        if @points.size == 1
          fail Error::InvalidGeometry, "LineString cannot have 1 point"
        end
      end
    ...
    module BasicLineMethods # :nodoc:
      def initialize(factory_, start_, end_)
        ...
        unless cstart_
          fail Error::InvalidGeometry, "Could not cast start: #{start_}"
        end
        cend_ = Feature.cast(end_, factory_, Feature::Point)
        fail Error::InvalidGeometry, "Could not cast end: #{end_}" unless cend_
      ...
      def _validate_geometry # :nodoc:
        super
        if @points.size > 2
          fail Error::InvalidGeometry, "Line must have 0 or 2 points"
        end
      end
      ...
      def _validate_geometry # :nodoc:
        ...
        if @points.size > 0
          ...
          if !@factory.property(:uses_lenient_assertions) && !is_ring?
            fail Error::InvalidGeometry, "LinearRing failed ring test"
          end
        end
      end
