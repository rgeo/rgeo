    ...
    class << self
      ...
      def projected_factory(opts_ = {})
        unless CoordSys::Proj4.supported?
          fail Error::UnsupportedOperation, "Proj4 is not supported because the proj4 library was not found at install time."
        end
        ...
        if (projection_factory_ = opts_[:projection_factory])
          ...
          unless projection_proj4_
            fail ::ArgumentError, "The :projection_factory does not have a proj4."
          end
          ...
          if projection_coord_sys_ && !projection_coord_sys_.is_a?(CoordSys::CS::ProjectedCoordinateSystem)
            fail ::ArgumentError, 'The :projection_factory\'s coord_sys is not a ProjectedCoordinateSystem.'
          end
          ...
          unless projection_proj4_
            fail ::ArgumentError, "Unable to determine the Proj4 for the projected coordinate system."
          end
          ...
          if projection_proj4_.is_a?(::String) || projection_proj4_.is_a?(::Hash)
            ...
            unless actual_projection_proj4_
              fail ::ArgumentError, "Bad proj4 syntax: #{projection_proj4_.inspect}"
            end
            projection_proj4_ = actual_projection_proj4_
          end
          if projection_coord_sys_ && !projection_coord_sys_.is_a?(CoordSys::CS::ProjectedCoordinateSystem)
            fail ::ArgumentError, "The :projection_coord_sys is not a ProjectedCoordinateSystem."
          end
