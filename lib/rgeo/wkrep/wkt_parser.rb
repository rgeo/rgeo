    ...
    class WKTParser
      ...
      def parse(str_)
        ...
        begin
          ...
          if @cur_token && !@ignore_extra_tokens
            fail Error::ParseError, "Extra tokens beginning with #{@cur_token.inspect}."
          end
        ensure
          _clean_scanner
        end
      ...
      def _check_factory_support # :nodoc:
        if @cur_expect_z && !@cur_factory_support_z
          fail Error::ParseError, "Geometry calls for Z coordinate but factory doesn't support it."
        end
        if @cur_expect_m && !@cur_factory_support_m
          fail Error::ParseError, "Geometry calls for M coordinate but factory doesn't support it."
        end
      end
      ...
      def _parse_type_tag(_contained_) # :nodoc:
        ...
        if zm_.length > 0 || @strict_wkt11
          ...
          if @cur_expect_z.nil?
            ...
            fail Error::ParseError, "Surrounding collection has Z but contained geometry doesn't."
          end
          expect_m_ = zm_[-1, 1] == "m" ? true : false
          if @cur_expect_m.nil?
            @cur_expect_m = expect_m_
          elsif expect_m_ != @cur_expect_m
            fail Error::ParseError, "Surrounding collection has M but contained geometry doesn't."
          end
        ...
        when "multipolygon"
          ...
          fail Error::ParseError, "Unknown type tag: #{type_.inspect}."
        end
      end
      ...
      def _parse_coords # :nodoc:
        ...
        if @cur_expect_z.nil?
          ...
          if num_extras_ > 0
            fail Error::ParseError, "Found #{extra_.size + 2} coordinates, which is too many for this factory."
          end
          _ensure_factory
        else
          val_ = 0
