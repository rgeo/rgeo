...
task build_ext: internal_ext_info_.map { |info_| info_[:staged_lib_path] } do
  internal_ext_info_.each do |info_|
    ...
    fail "Could not find target_prefix in makefile for #{info_[:name]}" unless target_prefix_
    fail "Could not find TARGET in makefile for #{info_[:name]}" unless target_name_
