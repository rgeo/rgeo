...
* The coordinate system WKT parser now recognizes the nonstandard "EXTENSION" node. This node is not part of the WKT, but it is present in some data sets, including the spatial_ref_sys table of PostGIS 2.0, so we now support it.
* Updated some of the other test cases to work with the data that ships with PostGIS 2.0. The test suite now passes when run against PostGIS 2.0 (but no longer 1.5.x).
