...
* Cloning a Geos factory caused a segfault. Fixed.
...
* Pure Ruby factories (simple_spherical, simple_cartesian) now support a :uses_lenient_assertions option, which disables assertion checking on LinearRing, Polygon, and MultiPolygon construction, since those checks can be expensive.
...
* Further work towards YAML and mashal serialization support. Factories are now done, but geometries are not.
